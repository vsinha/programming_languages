...
typedef enum {
    STACKVALUEROOT, HEAPLOCROOT, ENVROOT, EXPROOT, EXPLISTROOT,
    VALUELISTROOT, DEFROOT
} Rootalt;
...
struct Root {
    Rootalt alt;
    union {
        Value *stackvalueroot;
        Value * *heaplocroot;
        Env *envroot;
        Exp *exproot;
        Explist *explistroot;
        Valuelist *valuelistroot;
        Def *defroot;
    } u;
};
...
/*
 * Because potential roots include local variables of
 * C functions such as [[eval]], the set of roots
 * changes as the interpreter executes. In a compiled
 * system, the garbage collector is in cahoots with the
 * compiler, which knows where all the local variables
 * are. In fact, the compiler knows everything about the
 * layout of the activation records and the call stack.
 * A compiler can therefore arrange for a garbage
 * collector to be able to find roots easily, with no
 * added overhead during ``real'' computation. Since our
 * garbage collector is not in cahoots with the
 * C compiler, we need some other way of finding roots.
 * What we do is maintain an explicit root stack, in
 * addition to the C call stack, and we put extra code
 * in our C procedures to push roots onto and pop roots
 * off of the root stack.
 * <function prototypes for \uscheme>=
 */
void pushroot(Root r);
void poproot (Root r);
/*
 * It is a checked run-time error if the value passed to
 * [[poproot]] doesn't match the corresponding
 * [[pushroot]].
 */

/*
 * We make the allocator independent of initialization;
 * chunk [->] shows how to use [[allocloc]] to implement
 * [[allocate]].
 * <function prototypes for \uscheme>=
 */
Value *allocloc(void);
/*
 * [*] The allocator and root stack are related by this
 * precondition: Clients may call [[allocloc]] only when
 * all objects that could lead to live values have been
 * pushed on the root stack. The copying collector's
 * implementation of [[allocloc]] also requires that,
 * when called, all pointers to allocated values must be
 * reachable from the root stack, so that they can be
 * updated when the values move. Section [->] explains
 * the programming techniques we use to ensure that
 * these preconditions are satisfied.
 */

/*
 * There are a few more operations that affect the root
 * stack. Clients may use [[resetrootstack]] to clear
 * the root stack, e.g., in case an error unwinds the
 * C stack.
 * <function prototypes for \uscheme>=
 */
void resetrootstack(void);
/*
 * Collectors need to get at the roots on the stack.
 * Rather than define a procedural interface, we expose
 * the representation. The invariant is that for all 0
 * <=i < rootstacksize, [[rootstack[i]]] is a live root.
 * The representation is read only; it is an unchecked
 * run-time error for a client to modify [[rootstack]]
 * or [[rootstacksize]].
 * <function prototypes for \uscheme>=
 */
...
/*
 * Finally, the interface provides a routine to dump the
 * root information; [[printroots]] may help you debug
...
/*
 * To debug a collector, it can help to wrap values in
 * calls to [[validate]]. Calling [[validate(v)]]
 * returns [[v]], unless [[v]] is invalid, in which case
