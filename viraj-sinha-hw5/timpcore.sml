  ...
  let (* All literals are integers. \usetyLiteral     *)
      ...
      | ty (AGET (a, i))       = 
          let val tau_a = case ty a of
              ...
              val tau_i = ty a
          in
            if eqType(tau_i, INTTY) then (* i is the desired array index *)
              tau_a (* return type of the elements in the array *)
          ...
          in
            if eqType(tau_i, INTTY) then (* i is the desired array index *)
              if eqType(tau_a, tau_e) then  
                (* make sure type of array elements is the same as the 
                   element we're attempting to add to the array *)
                tau_e
