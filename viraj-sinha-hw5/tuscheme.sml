  ...
  case e of 
     ...
     | LETX (k, varlist, e) => 
       let fun letall(LET, varlist, ex) =
        ...
        | letall(LETSTAR, varlist, ex) =
              let fun recTyp ((var, exp)::tail, tenv) = 
                        recTyp(tail, bind(var, elabexp(exp, tenv, kindenv),tenv))
                    | recTyp (nil, tenv) = tenv
              in 
                elabexp(ex, recTyp(varlist, typenv), kindenv)
              end
     ...
     | LAMBDA (lambdalist, exp) => 
        let fun recTyp ((var, typ)::tail, tenv) = 
            ...
            val args = recTypList(lambdalist)
            val kinding = asTypeRec(lambdalist, kindenv)
            val exptype = elabexp(exp, recTyp(lambdalist, typenv), kindenv)
        in 
          funtype(args, exptype)
        end
...
fun elabdef (d, typenv, kindenv) = 
  case d of 
     ...
     | VALREC (x, tau_x, e) => 
         let val newenv = bind(x, tau_x, typenv)
             val tau_e = elabexp(e, newenv, kindenv)
         in 
           if (appearsUnprotectedIn(x, e)) then
               raise TypeError "x appears unprotected in e"
           else (newenv, typeString(tau_e))
         end
     ...
     | DEFINE (name, tau_name, lambdaexp) => 
         let val newenv = bind(name, tau_name, typenv)
             val tau_lambda = elabexp(LAMBDA(lambdaexp), newenv, kindenv)
         in 
           if (appearsUnprotectedIn(name, LAMBDA(lambdaexp))) then
               raise TypeError "x appears unprotected in e"
           else (newenv, typeString(tau_lambda))
         end
